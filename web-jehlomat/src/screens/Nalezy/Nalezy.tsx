...
const NavodLikvidace: FC = () => {
  ...
  const handleSelectAll = () => setSelected([ ...syringes ]);

  const isSelected = (syringe: ISyringe) => selected.some(s => s.id === syringe.id);
        ...
        <Grid container direction="row" alignItems="center" justifyContent="space-between">
          ...
          <Controls>
            {!isMobile && (
              <SearchInputDesktop 
                placeholder="Hledat"
                onChange={e => setSyringes(syringeMock.syringeList.filter(item => item.id.includes(e.target.value)))}
              />
            )}
              ...
              <ListItem key={item.id}>
                ...
                <ListItemCell syringe={item}>
                  {item.demolishedAt && item.demolished === true ? (
                    ...
                    <>{item.reservedTill ? <TextHighlight>Rezervováno TP</TextHighlight> :
                ...
                <ListItemCell syringe={item}>
                  ...
                  <ListItemMenu 
                    open={Boolean(anchorEl) && edit?.id === item.id} 
                    anchorEl={anchorEl!}
